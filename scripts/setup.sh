...
for script in implement check-tests check-workflow gh-aliases.sh update-workflow verify-links verify-repo; do
...
echo "       (or copy and customize: cp $TEMPLATE/skeleton/CLAUDE-template.md CLAUDE.md)"
echo "  4. Create GitHub labels (one-time per repo):"
echo "       $TEMPLATE/scripts/setup-github-labels.sh [OWNER/REPO]"
